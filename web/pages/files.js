...
function closeUploadModal() {
  ...
  const splitLongSectionsToggle = document.getElementById("splitLongSectionsToggle");
  if (splitLongSectionsToggle) splitLongSectionsToggle.checked = true;
...
const LONG_SECTION_SPLIT_WORD_THRESHOLD = 6000;
const ORPHAN_INTRO_WORD_LIMIT = 50;
const HEADING_TAGS = new Set(["h1", "h2", "h3", "h4", "h5", "h6"]);
const MEDIA_TAGS = new Set(["audio", "canvas", "embed", "iframe", "image", "img", "object", "picture", "svg", "video"]);
...
function countWordsInNode(node) {
  ...
  if (node.nodeType === Node.ELEMENT_NODE) {
    ...
    if (name === "script" || name === "style") return 0;
  }
  return countVisibleWords(node.textContent || "");
}

function findSplitChain(body, candidates, wordThreshold) {
  let best = null;
  let bestChildCount = 0;
  for (const node of candidates) {
    const childCount = node.childNodes.length;
    if (childCount <= bestChildCount || countWordsInNode(node) <= wordThreshold) continue;
    best = node;
  ...
  const chain = [];
  for (let node = best; node && node !== body; node = node.parentNode) chain.push(node);
  return chain.reverse();
}

function findIntroSplitChain(body, nodes, wordThreshold) {
  for (let index = 0; index < nodes.length; index++) {
    const node = nodes[index];
    if (node.nodeType !== Node.ELEMENT_NODE) continue;
    if (countWordsInNode(node) <= wordThreshold || node.childNodes.length <= 1) continue;
    ...
    return findSplitChain(body, [node, ...Array.from(node.getElementsByTagName("*"))], wordThreshold);
  }
  return [];
}

function mergeOrphanIntroChunks(chunks, wordThreshold) {
  ...
  for (let index = 0; index < chunks.length; index++) {
    ...
    if (index < chunks.length - 1 && isOrphanIntroChunk(chunk, wordThreshold)) {
      chunks[index + 1] = chunk.concat(chunks[index + 1]);
...
function isOrphanIntroChunk(nodes, wordThreshold) {
  const words = nodes.reduce((total, node) => total + countWordsInNode(node), 0);
  if (words > introWordLimit(wordThreshold)) return false;
  return words === 0 || nodes.some((node) => containsHeadingOrMedia(node));
}

function introWordLimit(wordThreshold) {
  return Math.max(1, Math.min(ORPHAN_INTRO_WORD_LIMIT, Math.floor(wordThreshold / 20)));
}

function containsHeadingOrMedia(node) {
  if (!node || node.nodeType !== Node.ELEMENT_NODE) return false;
  const elements = [node, ...Array.from(node.getElementsByTagName("*"))];
  return elements.some((el) => {
    const name = localName(el);
    return HEADING_TAGS.has(name) || MEDIA_TAGS.has(name);
  });
}

function allDecorative(nodes) {
  return nodes.every((node) => countWordsInNode(node) === 0 && !containsHeadingOrMedia(node));
}

function appendPrecedingSiblings(partDoc, target, originalNode) {
  const parent = originalNode.parentNode;
  if (!parent) return;
  for (const sibling of Array.from(parent.childNodes)) {
    if (sibling === originalNode) break;
    target.appendChild(partDoc.importNode(sibling, true));
  }
}
...
function uniqueOpfId(base, usedIds) {
  const clean = (base || "ci-section").replace(/[^A-Za-z0-9_.-]/g, "-");
  if (!usedIds.has(clean)) {
    usedIds.add(clean);
    return clean;
  }
  let index = 2;
  while (usedIds.has(clean + "-" + index)) index++;
  const id = clean + "-" + index;
  usedIds.add(id);
  return id;
}

function splitXhtmlContent(path, content, wordThreshold) {
  const parser = new DOMParser();
  const doc = parser.parseFromString(content, "application/xhtml+xml");
  if (doc.querySelector("parsererror")) return null;
  const root = doc.documentElement;
  const body = findFirstByLocalName(root, "body");
  if (!body) return null;
  if (countWordsInNode(body) <= wordThreshold) return null;
  let nodes = Array.from(body.childNodes);
  let splitChain = [];
  if (nodes.length <= 1) {
    splitChain = findSplitChain(body, Array.from(body.getElementsByTagName("*")), wordThreshold);
    if (splitChain.length > 0) {
      nodes = Array.from(splitChain[splitChain.length - 1].childNodes);
    } else {
      return null;
    }
  } else {
    splitChain = findIntroSplitChain(body, nodes, wordThreshold);
    if (splitChain.length > 0) {
      nodes = Array.from(splitChain[splitChain.length - 1].childNodes);
    }
  }
  ...
  for (const node of nodes) {
    ...
    if (current.length > 0 && currentWords + nodeWords > wordThreshold) {
      chunks.push(current);
      current = [];
      currentWords = 0;
    }
  ...
  if (mergedChunks.length <= 1) return null;

  const head = findFirstByLocalName(root, "head");
  const parts = [];
  const anchorToPart = {};
  const extIndex = path.lastIndexOf(".");
  const stem = extIndex === -1 ? path : path.substring(0, extIndex);
  const ext = extIndex === -1 ? ".xhtml" : path.substring(extIndex);

  mergedChunks.forEach((chunk, index) => {
    const partPath = index === 0 ? path : stem + "__ci_section_" + (index + 1) + ext;
    const partDoc = document.implementation.createDocument(inheritedNs([root], XHTML_NS), root.nodeName, null);
    const partRoot = partDoc.documentElement;
    copyAttributes(root, partRoot);
    if (head) partRoot.appendChild(partDoc.importNode(head, true));
    const partBody = partDoc.createElementNS(inheritedNs([body, root], XHTML_NS), body.nodeName);
    copyAttributes(body, partBody);
    let partContainer = partBody;
    for (const wrapper of splitChain) {
      if (index === 0) appendPrecedingSiblings(partDoc, partContainer, wrapper);
      const nextContainer = partDoc.createElementNS(inheritedNs([wrapper, body, root], XHTML_NS), wrapper.nodeName);
      copyAttributes(wrapper, nextContainer);
      partContainer.appendChild(nextContainer);
      partContainer = nextContainer;
    }
    for (const node of chunk) {
      partContainer.appendChild(partDoc.importNode(node, true));
      if (node.nodeType === Node.ELEMENT_NODE) {
        const elements = [node, ...node.querySelectorAll("[id],[name]")];
        for (const el of elements) {
          const id = el.getAttribute("id") || el.getAttribute("name");
          if (id) anchorToPart[id] = partPath;
        }
      }
    }
    partRoot.appendChild(partBody);
    parts.push({ path: partPath, content: serializer.serializeToString(partDoc) });
  });

  return { parts, anchorToPart };
}

function relocateReference(value, sourcePath, relocationMap) {
  if (!value || !value.includes("#")) return value;
  const splitAt = value.indexOf("#");
  const rawTarget = value.substring(0, splitAt);
  const anchor = value.substring(splitAt + 1);
  if (!anchor) return value;
  if (/^[A-Za-z][A-Za-z0-9+.-]*:/.test(rawTarget)) return value;
  const targetPath = rawTarget ? joinZipPath(dirname(sourcePath), decodeHref(rawTarget)) : sourcePath;
  const relocated = relocationMap[targetPath + "#" + decodeHref(anchor)];
  if (!relocated || relocated === targetPath) return value;
  return relativeZipPath(sourcePath, relocated) + "#" + anchor;
}

function rewriteAnchorReferencesInXml(path, content, relocationMap) {
  const parser = new DOMParser();
  const doc = parser.parseFromString(content, "application/xml");
  if (doc.querySelector("parsererror")) return { content, count: 0 };
  let count = 0;
  for (const el of Array.from(doc.getElementsByTagName("*"))) {
    for (const attr of Array.from(el.attributes || [])) {
      const name = attr.localName || attr.name;
      if (name !== "href" && name !== "src") continue;
      const next = relocateReference(attr.value, path, relocationMap);
      if (next !== attr.value) {
  ...
  return { content: safeSerialize(doc, content), count };
}

function splitLongEpubSections(xhtmlFiles, opfPath, opfContent, extraTextFiles = {}) {
  const parser = new DOMParser();
  const doc = parser.parseFromString(opfContent, "application/xml");
  if (doc.querySelector("parsererror"))
    return { xhtmlFiles, opfContent, extraTextFiles, filesSplit: 0, sectionsAdded: 0, linksRewritten: 0 };

  const manifest = findFirstByLocalName(doc, "manifest");
  const spine = findFirstByLocalName(doc, "spine");
  if (!manifest || !spine)
    return { xhtmlFiles, opfContent, extraTextFiles, filesSplit: 0, sectionsAdded: 0, linksRewritten: 0 };

  const opfDir = dirname(opfPath);
  const items = Array.from(manifest.children).filter((el) => localName(el) === "item");
  const itemById = {};
  ...
  const outputXhtml = { ...xhtmlFiles };
  const relocationMap = {};
  let filesSplit = 0;
  let sectionsAdded = 0;

  for (const itemref of Array.from(spine.children).filter((el) => localName(el) === "itemref")) {
    const idref = itemref.getAttribute("idref") || "";
    const item = itemById[idref];
    if (!item) continue;
    ...
    const xhtmlPath = joinZipPath(opfDir, href);
    const source = outputXhtml[xhtmlPath];
    if (!source) continue;

    const split = splitXhtmlContent(xhtmlPath, source, LONG_SECTION_SPLIT_WORD_THRESHOLD);
    if (!split || split.parts.length <= 1) continue;

    filesSplit++;
    sectionsAdded += split.parts.length - 1;
    for (const [anchor, partPath] of Object.entries(split.anchorToPart)) {
      relocationMap[xhtmlPath + "#" + anchor] = partPath;
    }
    for (const part of split.parts) {
      outputXhtml[part.path] = part.content;
    }

    const originalIndex = Array.from(spine.children).indexOf(itemref);
    for (let i = 1; i < split.parts.length; i++) {
      const part = split.parts[i];
      const partId = uniqueOpfId(idref + "-ci-section-" + (i + 1), usedIds);
      const partItem = doc.createElementNS(inheritedNs([item, manifest], OPF_NS), item.nodeName);
      partItem.setAttribute("id", partId);
      partItem.setAttribute("href", relativeZipPath(opfPath, part.path));
      partItem.setAttribute("media-type", item.getAttribute("media-type") || "application/xhtml+xml");
      manifest.appendChild(partItem);

      const partItemref = doc.createElementNS(inheritedNs([itemref, spine], OPF_NS), itemref.nodeName);
      for (const attr of Array.from(itemref.attributes || [])) {
        if (attr.name !== "idref") partItemref.setAttribute(attr.name, attr.value);
      }
      partItemref.setAttribute("idref", partId);
      const insertBefore = spine.children[originalIndex + i + 1] || null;
      spine.insertBefore(partItemref, insertBefore);
    }
  }

  let linksRewritten = 0;
  if (filesSplit > 0) {
    for (const [path, content] of Object.entries(outputXhtml)) {
      const rewritten = rewriteAnchorReferencesInXml(path, content, relocationMap);
      outputXhtml[path] = rewritten.content;
      linksRewritten += rewritten.count;
    }
    for (const [path, content] of Object.entries(extraTextFiles)) {
      const rewritten = rewriteAnchorReferencesInXml(path, content, relocationMap);
      extraTextFiles[path] = rewritten.content;
      linksRewritten += rewritten.count;
    }
  }

  return {
    xhtmlFiles: outputXhtml,
    opfContent: safeSerialize(doc, opfContent),
    extraTextFiles,
    filesSplit,
    sectionsAdded,
    linksRewritten,
...
async function convertEpubFile(file, progressCallback) {
  ...
  if (opfContent) {
    ...
    if (document.getElementById("splitLongSectionsToggle")?.checked) {
      const splitResult = splitLongEpubSections(processedXhtmlFiles, opfPath, t, extraTextFiles);
      processedXhtmlFiles = splitResult.xhtmlFiles;
      extraTextFiles = splitResult.extraTextFiles;
      t = splitResult.opfContent;
      if (splitResult.filesSplit > 0) {
        logFix(
          "Synthetic sections",
          `${splitResult.filesSplit} chapter(s), ${splitResult.sectionsAdded} new section(s)`,
        );
      }
    }
